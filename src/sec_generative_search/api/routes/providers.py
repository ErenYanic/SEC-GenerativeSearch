"""Provider key-validation route.

Single endpoint: ``POST /api/providers/validate``.  Wraps the
audit-logged :func:`validate_credential` seam so the SDK round-trip
that confirms a key is a thin route adapter, not a re-implementation.
